...
7. **Never `set -u` in a `Monitor` / background poll-script.** The background shell sources the host shell-snapshot, which references optional vars with no default (e.g. `ZSH_VERSION`). Under `set -u` (nounset) that becomes a fatal "unbound variable" error that floods stderr and stalls the loop — the script keeps running but never reaches its emit, so the watcher just times out with no event (a *silent* failure: the orchestrator sees no signal, not an error). Use `set -o pipefail` alone; never add `-u`. (This is the inverse of the usual "scripts should be strict" advice — a sourced-snapshot background shell is not a strict-clean environment.)

8. **Match poll markers case-insensitively, with no nested command substitution.** A poll-script that derives a grep anchor via a subshell/`tr` — e.g. `grep -E "^$(printf %s "$e" | tr a-z A-Z)_MARKER="` — is fragile: the nested `$(…)` / locale-sensitive `tr` parsed to a *mismatched* anchor in the background shell, so the marker never matched, the condition never became true, and the watcher silently timed out. Use a subshell-free, case-insensitive anchor instead: `grep -iE "^${e}_MARKER="`. Robust across shells and locales.

**On Hard Rules 7 + 8 — the silent-timeout failure class.** Both break a watcher the same way: the loop runs to its full timeout while *appearing* healthy, because the bug suppresses the emit rather than crashing. They are only safe to rely on when correctness does **not** depend on the watcher — i.e. the watcher is an *accelerator* over a resilient spine (e.g. a long `ScheduleWakeup` backstop that re-drives the work regardless). Both rules were surfaced by the IDEA-021 review-loop Monitor dogfood, where the resilient backstop absorbed two consecutive silent Monitor failures with zero damage — the design principle being **"correctness never depends on the accelerator."** Author background watchers to that standard: assume the watcher may silently never fire, and ensure a spine recovers if it doesn't.
