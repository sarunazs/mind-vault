...
- **Gotcha — the after-`$next()` pattern below misses thrown error responses** (429 / `abort(404)` / `abort(403)` / any `HttpException`): the throw unwinds past the middleware's post-`$next()` code, so error pages ship *without* the headers, and registering the middleware "first" does NOT fix it. Re-apply the triad in `bootstrap/app.php`'s `withExceptions(...->respond(...))` hook. Full explanation + both-places fix: [`references/HEADERS_ON_ERROR_RESPONSES.md`](references/HEADERS_ON_ERROR_RESPONSES.md).
