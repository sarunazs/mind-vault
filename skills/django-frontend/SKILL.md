...
- [Client markdown rendering of LLM output](references/CLIENT_MARKDOWN_RENDERING.md) — two traps rendering LLM/markdown in the browser: (1) the pre-markdown tag-stripper must be tag-SHAPED (`/<\/?[a-zA-Z!][^>]*>/g`) not "any `<…>`" (`/<[^>]*>/g`), else it eats prose arrows (`<-`,`<->`) and comparison operators (`a < b … c > d`) — XSS still stripped; (2) the model emits **fully-resolved URLs** (`/articles/5/`, same-origin absolute) not your `article:5` pseudo-scheme, so a link-reroute must reverse-match resolved same-origin URLs (superset of the scheme parser); persisted history re-renders client-side so one hook covers live + history.
