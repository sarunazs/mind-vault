...
- [references/DARK_DEPLOY_KILL_SWITCH.md](references/DARK_DEPLOY_KILL_SWITCH.md) — dark → shadow → live ladder; why a kill switch's OFF position must be faithful per call site; shadow OBSERVES but does not PROTECT (time in shadow has a running cost its own logs quantify); the staging unit is the ORGANISATION, not the tenant; rollback stops future writes but cannot unwind past ones
...
- [references/SHELL_INSTALLERS.md](references/SHELL_INSTALLERS.md) — authoring + review patterns for `install/install-*.sh` **and remote-sudo / forced-command wrappers**; installer-specific catalog (chown, marker blocks, opt-out sweep, target-user resolution) distilled from review-loop cycles across PRs #55/#58/#59, plus patterns 16–20 from br-docs IDEA-029 (`ssh -t` invisible sudo prompt, explicit-PTY vs forced-command keys, `nologin` breaks `command=`), with the language-general entries stubbed down into the `shell` layer
- [references/ROOTLESS_DOCKER.md](references/ROOTLESS_DOCKER.md) — rootless Docker deploys: non-login deploy shells (`screen … bash -c`, cron, systemd) miss the profile `DOCKER_HOST` and hit the dead rootful socket → idempotent deploy misreads "first-time". Fix with `docker context use rootless` (shell-independent) + a `DOCKER_HOST` auto-detect helper. Also: `docker exec` does NOT inherit the entrypoint's `umask` — exec-ed steps write group-read-only where the app's own writes are fine
- [references/ROOTLESS_DOCKER_OPENVZ.md](references/ROOTLESS_DOCKER_OPENVZ.md) — getting rootless Docker to *run at all* on a stripped cgroup-v1 OpenVZ VPS (no per-user systemd, no cgroup delegation, no FUSE, no persistent tun): the root-owned `docker-rootless.service` system-unit recipe + five compensations (static `crun --cgroup-manager=disabled`, tmpfs-over-`/sys/fs/cgroup`, `vfs` storage, `/dev/net/tun` tmpfiles, socket ACL). Probe per box (symptom, not `uname`); reserve for nodes where `systemctl --user` won't start
- [references/NGINX_TLS_REDIRECT_AND_CERTS.md](references/NGINX_TLS_REDIRECT_AND_CERTS.md) — redirect vhost + Let's Encrypt on a busy proxy: apex served by `default_server`/first-`ssl` fallthrough (additive vhost, not replace; wins by `server_name`/SNI precedence), two-phase `:80`→issue→append-`:443` (cert doesn't exist until issued), ACME-challenge location must stay a more-specific prefix than the 301 (longest-prefix, not file order) as a standing renewal precondition, renewer-unit varies (apt `certbot.timer` vs snap `snap.certbot.renew.timer` vs cron). Pairs with the shell maintenance-script contract
- [references/TRAEFIK_EDGE_HARDENING.md](references/TRAEFIK_EDGE_HARDENING.md) — public **Traefik v3** edge, no plugins: native **dotfile-404** (high-priority catch-all router + `ipAllowList`+`rejectStatusCode`, short-circuits before a dummy-server `noop` that must fail *closed*), the **`.well-known` carve-out** (RE2 has no lookahead → `&& !PathPrefix`; never block `/.well-known/acme-challenge/` or HTTP-01 renewal dies → cert expires → edge down), per-router rate-limit (omit `sourceCriterion`; `depth>0` trusts spoofable XFF), and the **version-bump audit drill** (latest-minor-only support, `acme.json` format unchanged v3.4–v3.7 → cert reused, `%2E` pre-routing normalization since v3.4.1, cert-reused-first gate + reviewed rollback), the **empty-config abort** (a programmatically-rendered fragment must emit bare `{}` when empty — `{"http":{"routers":{},"services":{}}}` is REJECTED and aborts the whole dynamic build, dropping every `@file` router estate-wide), and the **rate-limit threat model** (per-IP caps a single runaway IP, NOT a distributed botnet → set loose, a tight `burst` only 429s legit asset-heavy pages). Pairs with ROOTLESS_DOCKER.md and CONTAINER_SINGLE_FILE_MOUNT.md
- [references/CONTAINER_SINGLE_FILE_MOUNT.md](references/CONTAINER_SINGLE_FILE_MOUNT.md) — delivering ONE rendered file into a consumer container without silent staleness: a single-file / `volume.subpath` mount is **inode-bound**, so an atomic-rename writer (`write tmp; rename`) is pinned to the original inode and the consumer never sees updates (fix: in-place `O_TRUNC` write, or mount the directory); a **volume-at-a-directory shadows** the image's committed content — named-volume copy-up populates once then freezes, so rebuilt-image files never arrive (nested *explicit* binds DO compose; overlay one file over a committed placeholder instead); and the reviewer heuristic that catches both — **isolation-test the WRITE cycle, not just container startup** (startup reads the inode once and passes; the bug is the second write). Pairs with TRAEFIK_EDGE_HARDENING.md §6 empty-config abort
- [references/GITHUB_APP_DEPLOY_CREDENTIALS.md](references/GITHUB_APP_DEPLOY_CREDENTIALS.md) — replace personal SSH keys / PATs on servers with short-lived **GitHub App installation tokens minted on-box** (stateless git credential helper; openssl RS256 JWT → `access_tokens`, no `jq`/`gh`, no token at rest). The **two-Apps-not-one invariant** (an App's perms are the ceiling for any key-holder → read-only deploy box + write automation host need separate Apps), deploy (read-only) vs automation (write + gh-less PR) cases, the **unprivileged deploy-user wiring** (`!$HOME` shell-form + `useHttpPath` host-vs-path gotcha), and the gotchas (`workflows:write` for workflow files, Free-private repos don't enforce branch protection, agent hosts may be blocked from prod writes → human runs cutover). Pairs with ROOTLESS_DOCKER.md
