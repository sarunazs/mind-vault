...
- **Counter persistence** — the session commit counter, idle-poll counter, active-work-minutes tracker, `last_seen_copilot_signal_id` (per-engine slot under the shared-core refactor; formerly `last_seen_comment_id` in the pre-shared-core single-engine wrapper), `last_push_sha`, and **`no_progress_map` engine-namespaced under the shared-core refactor: `{ copilot: {<category>: <count>} }`** (not a flat per-category map; the namespace is mandatory so Phase 4's per-engine no-progress guard reads the correct slot) must be checkpointed to the scratch file after *every* mutation. **Path under the shared review-loop core (post-IDEA-005)**: `~/.claude/memory/projects/<project-slug>/review-loop-pr-<N>.md` (engine-agnostic; all engines' state in one file). The legacy per-engine path `copilot-pr-<N>.md` is retained here for historical context; new sessions write to the shared path. Conversation context can be summarised away across wake cycles; counters that live only in-context make the hard bounds unenforceable.
...
- **Zero Copilot activity for the current push SHA?** Request a Copilot review once (`./tools/copilot_retrigger.sh [PR_NUMBER]`, which wraps `gh pr edit <PR> --add-reviewer @copilot`), and **HARD SHORT-CIRCUIT** — skip the rest of triage (no Tier classification), skip PASS 2 (no patches), skip PASS 3 (no commit/push) entirely on this cycle. Jump directly to the wait/wake phase to poll for the eventual review. Do **not** fall through to "no findings, hand back" AND do not attempt to fabricate fixes — the trigger is in flight. Unlike Cursor Bugbot, the trigger is **not** a PR comment — it's a reviewer assignment via the GitHub API (no comment id to record in `last_seen_copilot_signal_id`; that field stays unset until the first review posts). From June 1, 2026 Copilot reviews consume GitHub Actions minutes; never re-trigger when Copilot activity already exists for the current push.
...
- Compare findings against `last_seen_copilot_signal_id` tracked in the scratch file (**not** last push SHA — if Phase 3 was skipped, the push doesn't advance, so a "since last push" comparison would stay true indefinitely and reset idle polls on every wake). If new findings → update `last_seen_copilot_signal_id`, reset idle polls, loop to PASS 1.
